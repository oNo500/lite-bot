...
function ChunkEditorContent({ documentId, data }: ChunkEditorContentProps) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setFlashedChunkId(null), 1500)
          ...
          <div className="divide-y">
            {data.chunks.map((chunk) => (
              <ChunkRow
                ...
                onHover={setHoveredChunkId}
                onToggleOpen={(open) => setOpenEditChunkId(open ? chunk.id : null)}
                rowRef={(el) => {
...
function buildSegments(rawContent: string, chunks: ChunkListItem[]): RenderSegment[] {
  ...
  for (const chunk of sorted) {
    if (chunk.charStart > cursor) {
      segments.push({ text: rawContent.slice(cursor, chunk.charStart) })
    }
    const start = Math.max(chunk.charStart, cursor)
    const end = Math.max(chunk.charEnd, start)
    if (end > start) {
      segments.push({ text: rawContent.slice(start, end), chunkId: chunk.id })
    }
      ...
      {segments.map((seg, i) => {
        ...
        return (
          <span
            ...
            className={className}
            onMouseEnter={() => onHover(seg.chunkId!)}
            onMouseLeave={() => onHover(null)}
            onClick={() => onClick(seg.chunkId!)}
          >
            {seg.text}
          </span>
        )
...
function ChunkRow({
  ...
  const collapsed = previewContent.length > 100 && !isOpen
  ...
  return (
    ...
    >
      ...
      {!isOpen && (
        <p className={`text-xs/relaxed wrap-break-word whitespace-pre-wrap ${chunk.enabled ? '' : 'text-muted-foreground line-through'}`}>
          {collapsed}
        </p>
      )}
    ...
    <div className="flex items-center gap-2">
      ...
      >
        ...
        <SelectContent>
          ...
          <SelectItem value="semantic" disabled>semantic（暂未实现）</SelectItem>
        </SelectContent>
      </Select>
      <Input
        type="number"
      ...
      <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            <DialogClose
              render={<Button variant="ghost" disabled={rechunk.isPending}>取消</Button>}
            />
