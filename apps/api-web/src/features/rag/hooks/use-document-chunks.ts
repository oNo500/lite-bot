...
export function useUpdateChunk(documentId: string) {
  ...
  return useMutation({
    ...
    onMutate: async ({ chunkId, patch }) => {
      ...
      if (patch.enabled === undefined || patch.editedContent !== undefined) return
      ...
      if (prev) {
        queryClient.setQueryData<DocumentChunksResult>(chunksKey(documentId), {
          ...prev,
          chunks: prev.chunks.map((c) => c.id === chunkId ? { ...c, enabled: patch.enabled! } : c),
        })
      }
      return { prev }
    },
...
export function useRechunkDocument(documentId: string) {
  ...
  return useMutation({
    mutationFn: async (config: ChunkConfig) => {
      ...
      if (!res.ok) {
        const err = await res.json().catch(() => ({ error: 'Re-chunk failed' })) as { error?: string }
